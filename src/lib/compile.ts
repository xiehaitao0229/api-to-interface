...
export default class Compile {
  ...
  apiUrl: string
  separator:  string                       // 类型分隔符，比如分号，逗号，空， '；' ' ，' '' 
  ...
  formatToTsInterface(source: any, interFaceName: string) {
    ...
    Object.keys(cloneData).forEach(item => {
      cloneDataStr += `\n  ${item}: ${cloneData[item]}${this.separator}`
    })
  ...
  jsonSchemaToTsInterface(source: any, interFaceName: string) {
    ...
    for(let key in source) {
      ...
      if (key !== 'required') {
        ...
        console.log(cloneData,'cloneData222')
      }
    ...
    Object.keys(cloneData).forEach(item => {
      ...
      if (cloneData[item].type) {
        cloneDataStr += `\n  ${item}${cloneData[item].required ? '' : '?'}: ${cloneData[item].type}${this.separator}`
      }
    })
  ...
  queryToTsInterface (source: any, interFaceName: string){
    ...
    source.forEach((item: any) => {
      ...
      str += `\n ${name}${required === '1' ? '' : '?'}: string${this.separator}`
    })
  ...
  generateExportStr(interFaceName:string, str:string) {
    ...
    // 直接使用``输出的空格缩进有问题
    this.exportStr = `export interface ` + interFaceName + ' {'
      + str +
    '\n}\n\n' + this.exportStr
  }
  ...
  */
  compile(jsonObj: any, interfaceName: string, compileType: string, apiName: string) {
    // 初始值
    this.exportStr = ''
    ...
    const resStr = `/** ${apiName}-${this.apiUrl} */\n${this.exportStr}`
    return resStr
  }
